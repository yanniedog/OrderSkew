...
- **Workspace design**: `workspace.css` owns tokens and the shell; `workspace.controls.css`, `workspace.results.css`, `workspace.responsive.css` and `styles.dialogs.css` separate controls, output, breakpoints and dialogs. The root page has its own header/footer and does not load the shared subpage frame. Keep calculator interactions instant. Check light/dark themes, direct mode buttons, mobile section links, visible settings, overview accounting, menus and dialogs after UI changes.
