...
- **Asset versions**: After changing a root calculator script or stylesheet, run `npm run assets:calculator` and include the updated `index.html`. Keep an explicit revalidation rule in `_headers` for every root calculator asset: query versions alone did not prevent a returning www session from retaining old files during a rollout. `npm run test:calculator` rejects stale asset references or missing revalidation rules. Hashes normalize CRLF/LF for portable checkouts.
